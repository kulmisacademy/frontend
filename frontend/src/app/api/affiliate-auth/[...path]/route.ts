...
/**
 * Proxies public affiliate auth routes to the Express API so the browser can
 * call same-origin `/api/affiliate-auth/*` (production-friendly, avoids CORS).
 */
async function proxy(req: NextRequest, ctx: { params: Promise<{ path: string[] }> }) {
  ...
  const target = `${base}/api/affiliate-auth/${suffix}${qs ? `?${qs}` : ""}`;
  ...
  try {
    ...
    return NextResponse.json(
      {
        error: "Affiliate auth proxy could not reach the backend API",
        detail: msg,
